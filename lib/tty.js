...
Server.prototype.initIO = function() {
  ...
  //   return self.handleAuth(socket.request, next);
  // });
...
Server.prototype.handleAuth = function(data, next) {
  ...
  next(null, true);
  /*
  jwt({ secret: new Buffer('JGLe0b9UkFBmmnlHioo2e48sAgLj84ZcGYiG_IqY6sw1-tstqGa_JlXZ-c-jEmA0', 'base64') })(data, null, function (err) {
    return !err
      ? next(null, true)
      : next(err);
  });
